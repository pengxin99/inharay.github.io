...
2. [Scheduler](#2-scheduler)
   ...
   2.2 [调度流程](#22-调度流程)
      ...
      2.2.2 [调度核心 schedule.schedule()](#222-调度核心-schedule.schedule)
...
6. [torch.distributed](#6-torch-distributed)
   ...
   6.4 [张量并行中的通信应用](#64-张量并行中的通信应用)
   6.5 [性能优化与特点](#65-性能优化与特点)
...
这个函数只在 decode 阶段被调用，用于判断是否可以往 seq 已分配的 block 中继续添加 token（decode 阶段每次只生成一个 token）。其逻辑是：
- 当 `len(seq) % self.block_size == 1` 时，表示添加新 token 会需要一个新的 block（当前序列长度是 block_size 的倍数 +1，添加后会开始新的 block）
- 此时需要至少有一个 free block 才能继续
- 当 `len(seq) % self.block_size != 1` 时，表示新 token 可以放入当前已分配的 block 中，不需要新的 block
- 此时不管是否有 free block 都可以继续

Scheduler 的高效运行离不开精心设计的数据结构支撑。无论是请求的表示、状态的跟踪，还是 KV 缓存块的管理，都依赖于 Nano-vLLM 中定义的核心数据结构。接下来，我们将详细分析这些数据结构的设计与实现。
...
在张量并行的实现中，`torch.distributed` 被广泛应用于不同类型的线性层：

1. **RowParallelLinear** 层：
